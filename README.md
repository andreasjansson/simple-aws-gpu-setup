...
The scripts and files I used to build the image are in the `bootstrap` directory. To build with headintheclouds:

    fab ensemble.up:build
    fab -R build bootstrap

Or you can just copy the files over from `bootstrap/files` and run the `bootstrap/bootstrap.sh` script yourself.
